...
remote_file '/opt/dynatrace/Dynatrace-OneAgent-Linux.sh' do
...
bash 'Install Dynatrace Agent' do
  cwd '/opt/dynatrace'
  code <<-EOH
    /bin/sh Dynatrace-OneAgent-Linux.sh APP_LOG_CONTENT_ACCESS=1 #{node['dvo_user']['dynatrace']['infra_only_linux']}
    EOH
